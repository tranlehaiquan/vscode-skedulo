...
import { AnyJson, ensureJsonMap, ensureString, isJsonMap } from "../types";
import { SkedError } from "../types/sked-error";

export type Tokens = Array<string | boolean | number | null | undefined>;

const getKey = (packageName: string, bundleName: string): string =>
...
const markdownLoader: FileParser = (
  filePath: string,
  fileContents: string
): StoredMessageMap => {
  const map = new Map<string, StoredMessage>();
  const hasContent = (lineItem: string): boolean =>
    !REGEXP_NO_CONTENT.exec(lineItem);

  // Filter out sections that don't have content
  const sections = fileContents
    .split(REGEXP_NO_CONTENT_SECTION)
    .filter(hasContent);
  ...
  for (const section of sections) {
    ...
    if (firstLine && rest.length > 0) {
      ...
      // use error instead of SkedError because messages.js should have no internal dependencies.
      throw new Error(
...
const jsAndJsonLoader: FileParser = (
  filePath: string,
  fileContents: string
): StoredMessageMap => {
  let json;
 ...
 * all plugins and consumers of cli-core. It is set up to handle localization down
...
export class Messages<T extends string> {
  ...
  private static loaders: Map<string, (locale: string) => Messages<string>> =
    new Map<string, (locale: string) => Messages<string>>();

  // A map cache of messages bundles that have already been loaded
  private static bundles: Map<string, Messages<string>> = new Map<
    string,
    Messages<string>
  >();
  ...
  public constructor(
    bundleName: string,
    locale: string,
    public readonly messages: StoredMessageMap
  ) {
  ...
  public static generateFileLoaderFunction(
    ...
    if (![".json", ".js", ".md"].includes(ext)) {
      throw new Error(
        `Only json, js and md message files are allowed, not ${ext}: ${filePath}`
      );
    }
  ...
  public static importMessagesDirectory(
    ...
    while (projectRoot.length >= 0) {
      try {
        ...
        if ((err as SkedError).code !== "ENOENT") throw err;
    ...
    if (!packageName) {
      ...
      try {
        packageName = _.toString(
          ensureJsonMap(
            Messages.readFile(path.join(moduleMessagesDirPath, "package.json"))
          ).name
        );

        if (!packageName) {
          throw new SkedError("MissingPackageName", errMessage);
        }
      } catch (err) {
        throw new SkedError("MissingPackageName", err as string);
      }
    }
  ...
  public static loadMessages(
    ...
    throw new SkedError(
      "MissingBundleError",
      `Missing bundle ${key} for locale ${Messages.getLocale()}.`
    );
  }
   ...
   */
  public static load<T extends string>(
    packageName: string,
    bundleName: string,
    keys: [T, ...T[]]
  ): Messages<T> {
   ...
   * @param exitCodeOrCause The exit code which will be used by SkedCommand or the underlying error that caused this error to be raised.
  ...
  public createError(
    ...
    return new SkedError(message, name, actions, exitCodeOrCause, cause);
  }

  /**
   * SkedError wants error names to end with the suffix Error.  Use this to create errors while preserving their existing name (for compatibility reasons).
   *
  ...
  public createWarning(
    ...
    return this.formatMessageContents({
      type: "warning",
      key,
      tokens,
      actionTokens,
  ...
  private formatMessageContents({
    ...
    const searchValue: RegExp =
      type === "error" ? /^error.*\./ : new RegExp(`^${type}.`);
    // Convert key to name:
    //     'myMessage' -> `MyMessageWarning`
    //     'myMessageError' -> `MyMessageError`
    //     'warning.myMessage' -> `MyMessageWarning`
    const name = `${_.upperFirst(key.replace(searchValue, ""))}${
      labelRegExp.exec(key) || preserveName ? "" : label
    }`;
    const message = this.getMessage(key, tokens);
    let actions;
    try {
      actions = this.getMessageWithMap(
        `${key}.actions`,
        actionTokens,
        this.messages
      );
  ...
  private getMessageWithMap(
    ...
    if (group) {
      ...
      if (childObject && isJsonMap(childObject)) {
        const childMap = new Map<string, StoredMessage>(
          Object.entries<StoredMessage>(childObject)
        );
    ...
    const messages = _.castArray(msg);
